<div class="profile-header">
    ...
    <figure class="profile-header-cover liquid">
        <img src="{{$user->cover ?? '/storage/default/user/profile/cover.jpg'}}" alt="cover-01">
    </figure>
    <!-- /PROFILE HEADER COVER -->
    ...
    <div class="profile-header-info">
        ...
        <div class="user-short-description big">
            ...
            <a class="user-short-description-avatar user-avatar big"  @if($user->username != 'soulite') href="{{Auth::user() == $user ? route('userProfileGet') : route('userPublicProfileGet', ['user' => $user->username])}}" @endif>
                <!-- USER AVATAR BORDER -->
                <div class="user-avatar-border">
                    <!-- HEXAGON -->
                    @if($user->username == 'soulite')
                    <style>
                        particle {
                          position: absolute;
                          left: 0;
                ...
                <div class="user-avatar-content" id="user-avatar-content">
                    <!-- HEXAGON -->
                    @if($user->username != 'soulite')
                    <div class="hexagon-image-100-110" data-src="{{$user->avatar ?? '/storage/default/user/profile/avatar.jpg'}}"></div>
                    @else
                    <img src="/assets/img/svg/tiger.svg" class="lit_bg">
                    <img src="/assets/img/svg/circles.svg" class="lit_lights">
                    <img src="{{$user->avatar ?? '/storage/default/user/profile/avatar.jpg'}}" class="lit_profile">
            ...
            <a class="user-short-description-avatar user-short-description-avatar-mobile user-avatar medium" @if($user->username != 'soulite') href="{{Auth::user() == $user ? route('userProfileGet') : route('userPublicProfileGet', ['user' => $user->username])}}" @endif>
                <!-- USER AVATAR BORDER -->
                <div class="user-avatar-border">
                    <!-- HEXAGON -->
                    @if($user->username == 'soulite')
                ...
                <div class="user-avatar-content">
                    ...
                    @else
                        <div class="hexagon-image-82-90" data-src="{{$user->avatar ?? '/storage/default/user/profile/avatar.jpg'}}"></div>
                    @endif
                    <!-- /HEXAGON -->
                </div>
                <!-- /USER AVATAR CONTENT -->
            ...
            <p class="user-short-description-title"><a href="{{Auth::user() == $user ? route('userProfileGet') : route('userPublicProfileGet', ['user' => $user->username])}}">{{$user->full_name ?? $user->username}}</a></p>
            <!-- /USER SHORT DESCRIPTION TITLE -->

            <!-- USER SHORT DESCRIPTION TEXT -->
            <p class="user-short-description-text"><a href="{{$user->website ?? '#'}}" target="_blank">{{parse_url($user->website)['host'] ?? ''}}</a></p>
            <!-- /USER SHORT DESCRIPTION TEXT -->
        </div>
        <!-- /USER SHORT DESCRIPTION -->

        <!-- PROFILE HEADER SOCIAL LINKS WRAP -->
        @include('home.user.profile.partial.widgets.profile-header-social')
        <!-- /PROFILE HEADER SOCIAL LINKS WRAP -->
        ...
        <div class="user-stats">
            ...
            <div class="user-stat big">
                <!-- USER STAT TITLE -->
                <p class="user-stat-title">{{$user->posts < 1000 ? $user->posts : number_format($user->posts/1000,1)."K"}}</p>
                <!-- /USER STAT TITLE -->
            ...
            <div class="user-stat big">
                <!-- USER STAT TITLE -->
                <p class="user-stat-title">{{$user->friends < 1000 ? $user->friends : number_format($user->friends/1000,1)."K"}}</p>
                <!-- /USER STAT TITLE -->
            ...
            <div class="user-stat big">
                <!-- USER STAT TITLE -->
                <p class="user-stat-title">{{$user->visits < 1000 ? $user->visits : number_format($user->visits/1000,1)."K"}}</p>
                <!-- /USER STAT TITLE -->
            ...
            <div class="user-stat big">
                ...
                <img class="user-stat-image" src="{{'/assets/img/flags/'.$user->country.'.png'}}" alt="flag {{$user->country}}">
                <!-- /USER STAT IMAGE -->
        ...
        <div class="profile-header-info-actions">
            @if( $addFriendButton ?? false)
            <!-- PROFILE HEADER INFO ACTION -->
            <!--<a href="{{route('userFriendshipAddGet', ['user' => $user->username])}}">
            <p class="profile-header-info-action button secondary"><span class="hide-text-mobile">Add</span> Friend +</p>
            </a>-->
                <a href="{{route('userFriendshipAddGet', ['user' => $user->username])}}" class="profile-header-info-action button secondary"><span class="hide-text-mobile">Add</span> Friend +</a>
            <!-- /PROFILE HEADER INFO ACTION -->
            @endif

            @if( $cancelFriendButton ?? false)
            <!-- PROFILE HEADER INFO ACTION -->

            <a href="{{route('userFriendshipCancelGet', ['user' => $user->username])}}" class="profile-header-info-action button tertiary"><span class="hide-text-mobile">Cancel</span> Friend +</a>

            <!-- /PROFILE HEADER INFO ACTION -->
            @endif

            @if( $acceptFriendButton ?? false)
            <!-- PROFILE HEADER INFO ACTION -->

            <a href="{{route('userFriendshipAcceptGet', ['user' => $user->username])}}" class="profile-header-info-action button secondary"><span class="hide-text-mobile">Accept</span> Friend +</a>

            <!-- /PROFILE HEADER INFO ACTION -->
            @endif

            @if(($deniedFriendButton ?? false) || ($blockFriendButton ?? false) || ($unblockFriendButton ?? false) || ($unfriendButton ?? false))
            <!-- PROFILE HEADER INFO ACTION -->
            ...
            <div class="widget-box-settings">
                ...
                <div class="post-settings-wrap">
                    ...
                    <div class="simple-dropdown widget-box-post-settings-dropdown profile-header-info-action-dropdown-widget">
                        @if($unfriendButton ?? false)
                        <!-- SIMPLE DROPDOWN LINK -->

                        <a href="{{route('userFriendshipUnfriendGet', ['user' => $user->username])}}" class="simple-dropdown-link">Unfriend</a>
                        <br>

                        <!-- /SIMPLE DROPDOWN LINK -->
                        @endif

                        @if($deniedFriendButton ?? false)
                        <!-- SIMPLE DROPDOWN LINK -->

                        <a href="{{route('userFriendshipDeniedGet', ['user' => $user->username])}}" class="simple-dropdown-link">Denied friend +</a>

                        <!-- /SIMPLE DROPDOWN LINK -->
                        @endif

                        @if($blockFriendButton ?? false)
                        <!-- SIMPLE DROPDOWN LINK -->

                        <a href="{{route('userFriendshipBlockGet', ['user' => $user->username])}}" class="simple-dropdown-link">Block</a>

                        <!-- /SIMPLE DROPDOWN LINK -->
                        @endif

                        @if($unblockFriendButton ?? false)
                        <!-- SIMPLE DROPDOWN LINK -->

                        <a href="{{route('userFriendshipUnblockGet', ['user' => $user->username])}}" class="simple-dropdown-link">Unblock</a>
            ...
            @if( $sendMessageButton ?? false)
            <!-- PROFILE HEADER INFO ACTION -->
            <p class="profile-header-info-action button primary" id="profile-header-send-message" data-user="{{$user->username}}"><span class="hide-text-mobile">Send</span> Message</p>
            <!-- /PROFILE HEADER INFO ACTION -->
            @endif
        </div>
        <!-- /PROFILE HEADER INFO ACTIONS -->
