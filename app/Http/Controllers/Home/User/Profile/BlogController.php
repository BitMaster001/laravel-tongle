...
class BlogController extends Controller
{
    ...
    public function get(Request $request){
        $user = Auth::user();
        return view("home.user.profile.blogs",['user' => $user, 'addFriendButton' => false, 'sendMessageButton' => false]);
    }
    ...
    public function getUser($user){
        ...
        return view("home.user.profile.blogs",['user' => $user, 'addFriendButton' => $addFriendButton, 'cancelFriendButton' => $cancelFriendButton, 'acceptFriendButton' => $acceptFriendButton, 'deniedFriendButton' => $deniedFriendButton, 'blockFriendButton' => $blockFriendButton, 'unblockFriendButton' => $unblockFriendButton, 'unfriendButton' => $unfriendButton, 'sendMessageButton' => $sendMessageButton]);

    }

    public function getBlog(Request $request, $blog){
        $user = Auth::user();
        $blog = Post::where('key', $blog)->first();
        if($blog == null || $blog->type != PostTypes::BLOG){
            abort(404);
        }
        return view("home.user.profile.blog",['user' => $user, 'addFriendButton' => false, 'sendMessageButton' => false, 'blog' => $blog]);
    }

    public function getUserBlog($user, $blog){
        $authUser = Auth::user();
        $user = User::where('username', $user)->first();
        if($authUser != null && $authUser->id == $user->id){
            return redirect(route('userAboutGet'));
        ...
        $blog = Post::where('key', $blog)->first();
        if($blog == null || $blog->user != $user || $blog->type != PostTypes::BLOG){
            abort(404);
        }

        return view("home.user.profile.blog",['user' => $user, 'addFriendButton' => $addFriendButton, 'cancelFriendButton' => $cancelFriendButton, 'acceptFriendButton' => $acceptFriendButton, 'deniedFriendButton' => $deniedFriendButton, 'blockFriendButton' => $blockFriendButton, 'unblockFriendButton' => $unblockFriendButton, 'unfriendButton' => $unfriendButton, 'sendMessageButton' => $sendMessageButton, 'blog' => $blog]);
