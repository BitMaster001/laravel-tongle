...
namespace App\Helpers\Friendships;
...
/**
 * Class Friendable
 * @package app\Helpers\Friendships
 */
trait Friendable
{
    /**
     ...
     *
     * @return \app\Models\Friendship|false
     */
    ...
    {
        return $this->findFriendship($recipient)->whereSender($recipient)->whereStatus(FriendshipStatus::PENDING)->exists();
    }
    ...
    {
        return Friendship::whereRecipient($recipient)->whereSender($this)->whereStatus(FriendshipStatus::PENDING)->exists();
    }
    ...
    {
        return $this->findFriendship($recipient)->where('status', FriendshipStatus::ACCEPTED)->exists();
    }
    ...
    {
        $updated = $this->findFriendship($recipient)->whereRecipient($this)->update([
            'status' => FriendshipStatus::DENIED,
        ]);
    ...
    public function groupFriend(Model $friend, $groupSlug)
    {
        ...
        $friendship       = $this->findFriendship($friend)->whereStatus(FriendshipStatus::ACCEPTED)->first();
    ...
    public function unblockFriend(Model $recipient)
    {
        ...
        $updated = $this->findFriendship($recipient)->whereSender($this)->update([
            'status' => FriendshipStatus::ACCEPTED,
        ]);
    ...
    {
        return $this->findFriendships(FriendshipStatus::PENDING, $groupSlug)->get();
    }
    ...
    {
        return $this->findFriendships(FriendshipStatus::ACCEPTED, $groupSlug)->get();
    }
    ...
    {
        return $this->findFriendships(FriendshipStatus::DENIED)->get();
    }
    ...
    {
        return $this->findFriendships(FriendshipStatus::BLOCKED)->get();
    }
    ...
    {
        return $this->friends()->whereRecipient($recipient)->whereStatus(FriendshipStatus::BLOCKED)->exists();
    }
    ...
    {
        return Friendship::whereRecipient($this)->whereStatus(FriendshipStatus::PENDING)->get();
    }
    ...
    {
        return Friendship::whereSender($this)->whereStatus(FriendshipStatus::PENDING)->get();
    }
    ...
    {
        $friendsCount = $this->findFriendships(FriendshipStatus::ACCEPTED, $groupSlug)->count();
        return $friendsCount;
    }
    ...
    public function canBefriend($recipient)
    {
        ...
        if ($friendship = $this->getFriendship($recipient)) {
            ...
            if ($friendship->status != FriendshipStatus::DENIED) {
                return false;
            }
        }
        return true;
    ...
    private function getFriendsQueryBuilder($groupSlug = '')
    {
        ...
        $friendships = $this->findFriendships(FriendshipStatus::ACCEPTED, $groupSlug)->get(['sender_id', 'recipient_id']);
    ...
    {
        $user1['friendships'] = $this->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
        $user1['recipients'] = $user1['friendships']->pluck('recipient_id')->all();
        $user1['senders'] = $user1['friendships']->pluck('sender_id')->all();

        $user2['friendships'] = $other->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
    ...
    {
        $friendships = $this->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
        $recipients = $friendships->pluck('recipient_id')->all();
        $senders = $friendships->pluck('sender_id')->all();
        ...
        $fofs = Friendship::where('status', FriendshipStatus::ACCEPTED)
...
//        );
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function isFriendsOfFriends(Model $recipient, $groupSlug = '')
    {
        $friendsOfFriendsId = $this->getFriendsOfFriendsIds($recipient);

        if (count($friendsOfFriendsId) > 0 ) return true;

        return false;
    }

    private function getFriendsOfFriendsIds(Model $recipient, $groupSlug = '')
    {
        $friendships = $this->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
        $recipients = $friendships->pluck('recipient_id')->all();
        $senders = $friendships->pluck('sender_id')->all();
        ...
        $recipientFriendships = $recipient->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
        $recipients = $recipientFriendships->pluck('recipient_id')->all();
        $senders = $recipientFriendships->pluck('sender_id')->all();

        $recipientFriendIds = array_unique(array_merge($recipients, $senders));

        return array_intersect($friendIds, $recipientFriendIds);
    }

    public function getFriendsIds($perPage = 0, $groupSlug = '')
    {
        $friendships = $this->findFriendships(FriendshipStatus::ACCEPTED)->get(['sender_id', 'recipient_id']);
        $recipients = $friendships->pluck('recipient_id')->all();
        $senders = $friendships->pluck('sender_id')->all();

        $friendIds = array_diff(array_unique(array_merge($recipients, $senders)), [$this->getKey()]);
