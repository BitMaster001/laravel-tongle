...
var map = L.Wrld.map("map", "3294271a8d486321eccb129e6acbf7a9", {
    center: lat_lng ?? [1.284038, 103.858903],
    zoom: 14
});


var user_icon = L.icon({
    iconUrl: '/assets/img/gif/try1.gif',
    iconSize: [64, 64],
});
...
var url = user_profile_url = container = '';
var userPoint = null;
var userFlag = '';
if(users.length > 0){
    users.forEach(function (item,key) {
        userPoint = L.marker([item.latitude, item.longitude], {
        ...
        //userFlag = "/assets/img/flags/" + item.country +".png";
        userFlag = (!item.country) ? 'singapore' : item.country.toLowerCase();
        userFlag = item.country ? "<span title='"+ userFlag +"' class='flag-moble country-flags flag-icon flag-icon-"+ getCountryCode(userFlag) +"' alt=''>" : '';
        user_profile_url = "<a href='"+url+"'><img alt='' src='" + (item.avatar ? item.avatar : '/storage/default/user/profile/avatar.jpg') +"' class='avatar avatar-96 photo avatar-default' height='96' width='96' />"+userFlag+"</a>";
        container = $('<div />');
        container.html(user_profile_url+"<a  href='"+url+"'><p style='word-break: break-all;'>"+ item.full_name  + "</p></a>");
        userPoint.bindPopup(container[0]);
    });
}
...
if(marketplaces.length > 0){
    marketplaces.forEach(function (item,key) {
        markerPoint = L.marker([item.user.latitude, item.user.longitude], {
            title: item.title,
            icon: user_icon
        }).addTo(markerClusterLayer);
        url = '/marketplace/'+ item.model.slag + '/' + item.key;
        user_profile_url = "<a class='speciallink' href='"+url+"'><img alt='' src='" + (item.images[0].src ? item.images[0].src : '') +"' class='avatar avatar-96 photo avatar-default' height='96' width='96' /></a>";
        container = $('<div />');
        container.html(user_profile_url+"" +
            "<a class='speciallink' href='"+url+"'>" +
            "<p style='word-break: break-all;'>"+ item.title +"</p>" +
            "<p style='word-break: break-all;'>$ "+ item.price +"</p>" +
            "</a>");
        markerPoint.bindPopup(container[0]);
    });
}
...
function marketPlaceFilters() {
    $('.map-search-type').on('change', function () {
        let filterSelector = $('.marketplace-filters');
        if(this.value == 1){
            filterSelector.removeClass('d-none');
        }else{
            filterSelector.addClass('d-none');
        }
    });
}
...
function getCountryCode(countryName){
    ...
    isoCountries.forEach(function(item, key){

        if(
            item.cname.toLowerCase().replace(' ', '').replace(',', '') ==
            countryName.toLowerCase().replace(' ', '').replace(',', '')
        ){
            code =  item.ccode.toLowerCase();
        }
    });
